...
def train_sae_pipeline(model, saes, cfg, pgn_chunks, all_moves_dict, elo_dict, num_epochs, buffer_size=8192, l1_coefficient=0.00001):
                    ...
                    if key in activations:
                        ...
                        if is_buffer_full:
                            ...
                            if total_sae_updates[key] % 500 == 0:
                                ...
                                if cfg.sae_attention_heads:    
                                    save_path = f'maia2-sae/sae/trained_saes_{cfg.test_year}-{formatted_month}-{cfg.sae_dim}-{cfg.l1_coefficient}-att-temp.pt'
                                if cfg.sae_residual_streams:
                                    save_path = f'maia2-sae/sae/trained_saes_{cfg.test_year}-{formatted_month}-{cfg.sae_dim}-{cfg.l1_coefficient}-res-temp.pt'
                                if cfg.sae_mlp_outputs:
                                    save_path = f'maia2-sae/sae/trained_saes_{cfg.test_year}-{formatted_month}-{cfg.sae_dim}-{cfg.l1_coefficient}-mlp-temp.pt'
...
def parse_args(args=None):
    ...
    parser.add_argument('--end_month', default=10, type=int)
...
if __name__ == '__main__':
    ...
    while (current_year < cfg.end_year) or (current_year == cfg.end_year and current_month <= cfg.end_month):
        ...
        try:
            ...
            trained_saes = train_sae_pipeline(model, saes, cfg, pgn_chunks, all_moves_dict, elo_dict, num_epochs=cfg.num_sae_epochs, buffer_size=8192, l1_coefficient=cfg.l1_coefficient)
    ...
    if cfg.sae_attention_heads:    
        save_path = f'maia2-sae/sae/trained_saes_{cfg.test_year}-{formatted_month}-{cfg.sae_dim}-{cfg.l1_coefficient}-att.pt'
    if cfg.sae_residual_streams:
        save_path = f'maia2-sae/sae/trained_saes_{cfg.test_year}-{formatted_month}-{cfg.sae_dim}-{cfg.l1_coefficient}-res.pt'
    if cfg.sae_mlp_outputs:
        save_path = f'maia2-sae/sae/trained_saes_{cfg.test_year}-{formatted_month}-{cfg.sae_dim}-{cfg.l1_coefficient}-mlp.pt'
    torch.save(sae_state_dicts, save_path)
