...
def is_square_under_defensive_threat(board: chess.Board, square_index: int) -> bool:
    """Check if a white piece on the square is under threat from black."""
...
def is_square_under_offensive_threat(board: chess.Board, square_index: int) -> bool:
    """Check if white has an attacking advantage on a square with a black piece."""
    piece = board.piece_at(square_index)
...
def create_chessboard_heatmap(correlations_dict: dict, title: str) -> None:
    ...
    output_file = f'maia2-sae/figs/squarewise_threats/awareness_correlation_{title.lower().replace(" ", "_")}.pdf'
    plt.savefig(output_file, bbox_inches='tight', dpi=300)
    plt.close()

def analyze_awareness_correlations(
    positions_path: str,
    ...
    """Analyze correlations between awareness features and best move probabilities."""
    ...
    # Process each square for defensive and offensive awareness
    for square_name in tqdm.tqdm(chess.SQUARE_NAMES, desc="Processing squares"):
        ...
        def_key = f"defensive_awareness_{square_name}"
        off_key = f"offensive_awareness_{square_name}"
        ...
        # Process positions
        feature_activations = {
            'defensive': {'layer6': [], 'layer7': []},
            'offensive': {'layer6': [], 'layer7': []}
        }
        best_move_probs = {'defensive': [], 'offensive': []}
        ...
        for pos_idx, position in enumerate(positions):
            ...
            if is_square_under_defensive_threat(board, check_square):
                def_positions.append((board.fen(), best_move, 3, 3, 1))
                original_indices['defensive'].append(pos_idx)
                
            # Check offensive threat
            if is_square_under_offensive_threat(board, check_square):
                off_positions.append((board.fen(), best_move, 3, 3, 1))
...
if __name__ == "__main__":
    ...
    os.makedirs('maia2-sae/figs/squarewise_threats_awareness', exist_ok=True)
    ...
    # Run analysis
    results = analyze_awareness_correlations(
        positions_path='maia2-sae/transmono_positions_for_correlation.pkl',
        layer6_features_path='maia2-sae/dataset/intervention/layer6_defensive_awareness.pickle',
        layer7_features_path='maia2-sae/dataset/intervention/layer7_defensive_awareness.pickle',
        model_path='maia2-sae/weights.v2.pt',
        sae_path='maia2-sae/sae/best_jrsaes_2023-11-16384-1-res.pt',
        device=device
    )
    
    # Create heatmaps for each layer and awareness type
    for layer in ['layer6', 'layer7']:
        for awareness_type in ['defensive', 'offensive']:
            create_chessboard_heatmap(
                results[layer][awareness_type],
                f"{awareness_type.capitalize()} Awareness Correlations ({layer})"
            )
    ...
    for layer in ['layer6', 'layer7']:
        for awareness_type in ['defensive', 'offensive']:
            correlations = np.array(list(results[layer][awareness_type].values()))
            correlations = correlations[~np.isnan(correlations)]  # Remove NaN values
            
            print(f"\n{awareness_type.capitalize()} {layer}:")
    ...
    # Save results
    output_path = 'maia2-sae/defensive_awareness_square_correlations_by_layer.pkl'
