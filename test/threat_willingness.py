...
def is_attacking_opportunity_taken(fen: str, move: str, square_index: int) -> bool:
    ...
    Returns True if the move takes advantage of an attacking opportunity against a black piece.
    """
    board = chess.Board(fen)
    piece_before_move = board.piece_at(square_index)
    ...
    # Check if there was an attacking opportunity before the move
    ...
    # Check if the move captures the piece at the square
...
def initialize_concept_functions() -> Tuple[List[str], Dict[str, Any]]:
    ...
    for square in range(64):
        ...
        defensive_key = f"is_piece_no_longer_under_attack_{square_name}"
        defensive_func = partial(is_piece_no_longer_under_attack, square_index=square)
        function_map[defensive_key] = defensive_func
        move_dependent_header.append(defensive_key)
        
        # Offensive willingness
        offensive_key = f"is_attacking_opportunity_taken_{square_name}"
        offensive_func = partial(is_attacking_opportunity_taken, square_index=square)
...
def main():
    ...
    # Save results separately for defensive and offensive willingness
    for layer_key in target_key_list:
        layer_name = 'layer6' if 'block 0' in layer_key else 'layer7'
        
        # Split results into defensive and offensive willingness
        defensive_results = {k: v for k, v in results[layer_key].items() 
                           if k.startswith('is_piece_no_longer_under_attack_')}
        offensive_results = {k: v for k, v in results[layer_key].items() 
                           if k.startswith('is_attacking_opportunity_taken_')}
        
        # Save each type of results
        with open(f'maia2-sae/dataset/intervention/{layer_name}_defensive_willingness.pickle', 'wb') as f:
            pickle.dump(defensive_results, f)
            
        with open(f'maia2-sae/dataset/intervention/{layer_name}_offensive_willingness.pickle', 'wb') as f:
